...
// forno caps eth_getLogs at a 5000-block range and rate-limits sustained sweeps.
const CHUNK = BigInt(process.env.CHUNK || "5000");
const PACE_MS = Number(process.env.PACE_MS || "150");
...
async function getAllLogs(event) {
  ...
  for (let from = START_BLOCK; from <= latest; from += CHUNK) {
    ...
    for (let attempt = 0; ; attempt++) {
      try {
        logs = await client.getLogs({ address: FLIP_ADDRESS, event, fromBlock: from, toBlock: to });
        break;
      } catch (e) {
        const rateLimited = (e?.details || e?.message || "").toLowerCase().includes("rate");
        if (attempt >= 8) throw e;
        await sleep(rateLimited ? 20000 : 1500); // back off hard on forno's per-window rate limit
      }
    }
    out.push(...logs);
...
async function main() {
  ...
  try {
    ...
    // Baseline for the live /api/stats route: it seeds aggregates from here and
    // only scans blocks AFTER `block`, so each request stays under the rate limit.
    // Includes the full player set so unique-player counts stay exact.
    const baseline = {
      block: generatedAtBlock,
      flips: flips.length,
      wins,
      wageredWei: totalWagered.toString(),
      players: [...players].sort(),
    };
    writeFileSync(join(HERE, "..", "frontend", "lib", "onchainBaseline.json"), JSON.stringify(baseline));
